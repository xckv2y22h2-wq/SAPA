...
from models.prompters import TokenPrompter, PromptLearner
...
Batch_size=256, Dataset=ImageNet, train_numsteps=2, train_stepsize==train_eps
add_prompt_size=100  
learning_rate=40

Default Evaluation Setting: 
20-step PGD  test_stepsize==1 
---------------------------------

# TeCoA (ICLR 2023)
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --epochs 10 --train_eps 1 --vpt_method TeCoA --exp_name VPT_TeCoA_epoch10_eps1

# PMG (CVPR 2024)
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --epochs 10 --train_eps 1 --vpt_method PMG --W_Pred_Align_Ori 1.0 --W_Pred_Align 1.0 --exp_name VPT_PMG_epoch10_eps1

# Adv_Img + Adv_Text (Joint tuning PGD-AT)
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --epochs 10 --train_eps 1 --vpt_method ImgText_PGD --W_Pred_Align_Ori 1.0 --adv_prompt_gen True --text_perb_stepsize 1e-4 --exp_name VPT_ImgText_PGD_epoch10_eps1

# TRADES (Nat_CE + beta * KL(adv, ori_nat))
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --epochs 10 --train_eps 1 --vpt_method TRADES --W_Pred_Align_Ori 9.0 --exp_name VPT_TRADES90_epoch10_eps1
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --epochs 10 --train_eps 1 --vpt_method TRADES --W_Pred_Align_Ori 9.0 --mul_noise_beta 0.05 --exp_name VPT_TRADES90_mulbeta005_epoch10_eps1

----------------------
More Epsilon AT:
CUDA_VISIBLE_DEVICES=4,5 python new_visual_prompt.py --epochs 10 --train_eps 2 --train_stepsize 2 --test_eps 2 --vpt_method TeCoA --exp_name VPT_TeCoA_epoch10_eps2
CUDA_VISIBLE_DEVICES=4,5 python new_visual_prompt.py --epochs 10 --train_eps 3 --train_stepsize 3 --test_eps 3 --vpt_method TeCoA --exp_name VPT_TeCoA_epoch10_eps3
CUDA_VISIBLE_DEVICES=4,5 python new_visual_prompt.py --epochs 10 --train_eps 4 --train_stepsize 4 --test_eps 4 --vpt_method TeCoA --exp_name VPT_TeCoA_epoch10_eps4

CUDA_VISIBLE_DEVICES=6,7 python new_visual_prompt.py --epochs 10 --train_eps 2 --train_stepsize 2 --test_eps 2 --vpt_method PMG --W_Pred_Align_Ori 1.0 --W_Pred_Align 1.0 --exp_name VPT_PMG_epoch10_eps2
CUDA_VISIBLE_DEVICES=6,7 python new_visual_prompt.py --epochs 10 --train_eps 3 --train_stepsize 3 --test_eps 3 --vpt_method PMG --W_Pred_Align_Ori 1.0 --W_Pred_Align 1.0 --exp_name VPT_PMG_epoch10_eps3
CUDA_VISIBLE_DEVICES=6,7 python new_visual_prompt.py --epochs 10 --train_eps 4 --train_stepsize 4 --test_eps 4 --vpt_method PMG --W_Pred_Align_Ori 1.0 --W_Pred_Align 1.0 --exp_name VPT_PMG_epoch10_eps4

----------------------
CUDA_VISIBLE_DEVICES=0,1 python new_visual_prompt.py --evaluate --eval_type full --test_eps 1 --resume XXXX/model_best.pth.tar
...
def parse_option():
    ...
    parser.add_argument('--image_size', type=int, default=224,
                        help='image size')
    parser.add_argument('--imagenet_root', type=str, default='./datasets/ImageNet/')
    ...
    # Extra modules
    parser.add_argument('--vpt_method', type=str, default='TRADES',
                        help='TeCoA|PMG|ImgText_PGD|TRADES')
...
def main():
    ...
    args.imgnet_full = imagenet_root
    ...
    # Load original CLIP model without additional visual token prompts
    original_model = None
    if args.W_Pred_Align_Ori > 0.0:
        original_model, preprocess = clip.load('ViT-B/32', device, jit=False, prompt_len=0)
        convert_models_to_fp32(original_model)
        original_model = torch.nn.DataParallel(original_model)  # .to(device)
        original_model.eval()

    ### !!! These two are prompters for images
    ...
    if args.evaluate:
        if args.eval_type == 'fast':
            ...
            val_dataset_name = ['ImageNet', 'STL10', 'cifar10', 'cifar100', 
    ...
    texts_train, training_original_classnames = get_text_prompts_train(args, train_dataset, template=template)
...
def validate(val_loader_list, val_dataset_name, texts_list, model,
    ...
    for cnt in range(dataset_num):
        ...
        progress = ProgressMeter(
            len(val_loader),
            [batch_time, losses, top1_prompt, top1_adv_prompt],
        ...
        for i, (images, target) in enumerate(tqdm(val_loader, ncols = 80)):
            ...
            with autocast():
                ...
                if args.attack == 'CW':
                    ...
                    attacked_images = attack_auto_new(model, images, target, text_tokens, prompter, add_prompter, 
                                                      epsilon=args.test_eps, attacks_to_run=attacks_to_run)
                else:
                ...
                with torch.no_grad():
                    prompt_token = add_prompter()
                    output_prompt_adv, _ = multiGPU_CLIP(model, prompter(clip_img_preprocessing(attacked_images)),
                                                            text_tokens, prompt_token)
        ...
        if args.evaluate:
            log_record(dataset_name + '--- Clean prompt Acc.: {top1_prompt.avg:.2f}  Adv prompt Acc.: {top1_adv_prompt.avg:.2f}.'
                .format(top1_prompt=top1_prompt, top1_adv_prompt=top1_adv_prompt), args.val_log_name)
        else:
            print(dataset_name + '--- Clean prompt Acc.: {top1_prompt.avg:.2f}  Adv prompt Acc.: {top1_adv_prompt.avg:.2f}.'
                .format(top1_prompt=top1_prompt, top1_adv_prompt=top1_adv_prompt))
                      
        # acc_all_nat.append(top1_org.avg)
        acc_all_prompt_nat.append(top1_prompt.avg)
        # acc_all_adv.append(top1_adv_org.avg)
        acc_all_prompt_adv.append(top1_adv_prompt.avg)

    if args.evaluate:
        log_record('Average on all datasets --- Clean prompt Acc.: {:.2f}  Adv prompt Acc.: {:.2f}.'
                .format(np.mean(acc_all_prompt_nat), np.mean(acc_all_prompt_adv)), args.val_log_name)
    else:
        print('Average on all datasets --- Clean prompt Acc.: {:.2f}  Adv prompt Acc.: {:.2f}.'
                .format(np.mean(acc_all_prompt_nat), np.mean(acc_all_prompt_adv)))

    return np.mean(acc_all_prompt_adv)
